...
def get_inverse(J: Jacobian) -> Jacobian:
  """
  Return the inverse Jacobian object.

  If J encodes the derivatives of a coordinate map z(x), with
    - J.value[a, i]   = ∂z^a / ∂x^i,
    - J.gradient[a, i, j] = ∂²z^a / ∂x^i ∂x^j,
  ...
  B = J.gradient  # shape (a, j, k) if present
  C = J.hessian   # shape (a, j, k, l) if present
