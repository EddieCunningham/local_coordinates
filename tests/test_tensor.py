...
def test_lie_bracket():
  ...
  coord_vector_jet = function_to_jet(inv_chart, x0)

  # Create BasisVectors objects
  p = jnp.array([0., 0.]) # this is arbitrary for this test
  basis = BasisVectors(p=p, components=coord_vector_jet.get_gradient_jet())

  X: TangentVector = basis.get_basis_vector(0)
  Y: TangentVector = basis.get_basis_vector(1)

  lb = lie_bracket(X, Y)

  # The Lie bracket between coordinate basis vectors is zero
