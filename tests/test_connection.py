...
def make_vector(basis: BasisVectors, comps: jnp.ndarray, grad: jnp.ndarray | None) -> Tensor:
  ttype = TensorType(1, 0)
  jet = Jet(value=comps, gradient=grad, hessian=None)
...
def test_covariant_derivative_linearity_fY():
    ...
    # RHS: X(f)Y + f(∇_X Y)
    @jet_decorator
    def X_f_components(T: Tensor, f: Jet) -> Jet:
      return jnp.einsum('i,i', f.gradient, T.components.value)
    X_f = X_f_components(X, f_jet)
